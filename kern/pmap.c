...
mem_init(void)
{
	...
	//cprintf("debug kern_pgdir is: %08x\n",(uint32_t)kern_pgdir);
	//////////////////////////////////////////////////////////////////////
	...
	// Your code goes here:
	boot_map_region(kern_pgdir,(uintptr_t)UPAGES,PTSIZE,PADDR(pages),PTE_U);
	//////////////////////////////////////////////////////////////////////
	...
	//boot_map_region(kern_pgdir,(uintptr_t)KERNBASE,(0xffffffff-KERNBASE)+1,0,PTE_W);
	
	
	
	
	boot_map_region_large(kern_pgdir,(uintptr_t)KERNBASE,(0xffffffff-KERNBASE)+1,0,PTE_W);
...
page_free(struct Page *pp)
{
	// Fill this function in
	if(pp->pp_ref!=0){
		panic("Page's refcnt is nonzero.\n");
	}
...
{
	// Fill this function in
	pde_t *pd_p = pgdir+PDX(va);
	pte_t *pt_p;
	if(*pd_p & PTE_P){
		pt_p = (pte_t *)KADDR(PTE_ADDR(*pd_p));
		return pt_p + PTX(va);
	}
	else{
		if(!create){
			...
			if((newp = page_alloc(1))==NULL){
				return NULL;
			}
			newp->pp_ref++;
			*pd_p = page2pa(newp) | PTE_P | PTE_U | PTE_W;
			pt_p = (pte_t *)KADDR(PTE_ADDR(*pd_p));
			return pt_p + PTX(va);
		}
	}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	size_t i;
	for(i=0;i<page_num;i++,va+=PGSIZE,pa+=PGSIZE){
		pte_t *pt_p = pgdir_walk(pgdir,(void *)va,1);
		*pt_p = pa | PTE_P | perm;
	}
...
{
	// Fill this function in
	size_t page_num = size / PTSIZE;
	size_t i;
	for(i=0;i<page_num;i++,va+=PTSIZE,pa+=PTSIZE){
		pde_t *pd_p = pgdir+PDX(va);
		*pd_p = pa | PTE_P | PTE_PS |perm;
	}
}
...
{
	// Fill this function in
	pte_t *pt_p = pgdir_walk(pgdir,va,1);
	if(pt_p == NULL){
		return -E_NO_MEM;
	}
	if(*pt_p & PTE_P){
		if(page2pa(pp) == PTE_ADDR(*pt_p)){
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	// Fill this function in
	pte_t *pt_p;
	pt_p = pgdir_walk(pgdir, va, 0);
	if(pt_p == NULL || !(*pt_p & PTE_P)){
		return NULL;
	}
	if(pte_store !=0){
		*pte_store = pt_p;
	}
	return pa2page(PTE_ADDR(*pt_p));
}
